...
/**
 * Round a $/million-token price to 6 decimals.
 *
 * Upstream publishes $/token, which we scale by 1_000_000; binary floating
 * point leaves dust behind (8e-7 * 1e6 === 0.7999999999999999). The cheapest
 * real price in the catalog is 0.001 and the finest genuine granularity is
 * 0.003625, so 6 decimals is lossless for actual pricing and only trims the
 * artifacts.
 */
...
`;
			...
			output += `\t\t\tcost: {\n`;
			output += `\t\t\t\tinput: ${price(model.cost.input)},\n`;
			output += `\t\t\t\toutput: ${price(model.cost.output)},\n`;
			output += `\t\t\t\tcacheRead: ${price(model.cost.cacheRead)},\n`;
			output += `\t\t\t\tcacheWrite: ${price(model.cost.cacheWrite)},\n`;
			output += `\t\t\t},\n`;
